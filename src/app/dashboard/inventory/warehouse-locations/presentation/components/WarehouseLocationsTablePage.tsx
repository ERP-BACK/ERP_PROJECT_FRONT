...
export function WarehouseLocationsTablePage() {
  ...
  const [editingItem, setEditingItem] = useState<WarehouseLocation | null>(
    null,
  );
  const warehouseLocationsHeader = {
    filters: [
      {
        title: "Filtros",
  ...
  return (
    <>
      <PageHeader pageHeader={warehouseLocationsHeader} />

      <Show
        when={!isLoading}
        fallback={<TableSkeleton columns={columnsWarehouseLocations.length} />}
      >
        <MainDataTable
