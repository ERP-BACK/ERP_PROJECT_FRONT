...
import { columnsSalesChannels } from "./columns-sales-channel";
import { salesChannelFormConfig } from "../forms/sales-channel-form.config";
...
export function SalesChannelsTablePage() {
  const {
    data,
    isLoading,
    pagination,
  ...
  const [editingItem, setEditingItem] = useState<SalesChannel | null>(null);

  const salesChannelsHeader = {
    filters: [
      {
        title: "Filtros",
  ...
  const columnsWithActions = [
    ...columnsSalesChannels,
    {
      id: "actions",
      header: "Acciones",
      cell: ({ row }: { row: { original: SalesChannel } }) => (
  ...
  return (
    <>
      <PageHeader pageHeader={salesChannelsHeader} />

      <Show
        when={!isLoading}
        fallback={<TableSkeleton columns={columnsSalesChannels.length} />}
      >
        <MainDataTable
      ...
      <CrudFormDialog
        ...
        title={editingItem ? "Editar Canal de Ventas" : "Crear Canal de Ventas"}
        formConfig={salesChannelFormConfig}
        defaultValues={
          editingItem
      ...
      <ExcelImportDialog
        ...
        moduleKey="salesChannel"
        title="Importar Canales de Ventas desde Excel"
        onSuccess={() => {}}
      />
    </>
  );
