...
import { columnsTaxCodes } from "./columns-tax-code";
import { taxCodeFormConfig } from "../forms/tax-code-form.config";
...
export function TaxCodesTablePage() {
  const {
    data,
    isLoading,
    pagination,
  ...
  return (
    <>
      <PageHeader pageHeader={taxCodesHeader} />

      <Show
        when={!isLoading}
        fallback={<TableSkeleton columns={columnsTaxCodes.length} />}
      >
        <MainDataTable
      ...
      <CrudFormDialog
        ...
        title={editingItem ? "Editar Código de Impuesto" : "Crear Código de Impuesto"}
        formConfig={taxCodeFormConfig}
        defaultValues={
          editingItem
      ...
      <ExcelImportDialog
        ...
        moduleKey="taxCode"
        title="Importar Códigos de Impuesto desde Excel"
        onSuccess={() => {}}
      />
    </>
  );
