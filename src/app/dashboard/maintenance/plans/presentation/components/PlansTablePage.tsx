...
export function PlansTablePage() {
  ...
  const { data, isLoading, pagination, setPagination, deleteMutation } = usePlans();

  const activateMutation = useMutation({
    mutationFn: planActions.activate,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["maintenance-plans"] });
      toast({ title: "Plan activado", description: "El plan se ha activado correctamente" });
    },
    onError: (error: Error) => {
  ...
  const deactivateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["maintenance-plans"] });
      toast({ title: "Plan desactivado", description: "El plan se ha desactivado correctamente" });
    },
    onError: (error: Error) => {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    },
  ...
  const criticalPlans = plans.filter((p) => p.priority === "critical" || p.priority === "high").length;
  const dueSoonPlans = plans.filter((p) => {
    if (!p.next_due_date) return false;
    const daysUntil = Math.ceil(
      (new Date(p.next_due_date).getTime() - Date.now()) / (1000 * 60 * 60 * 24)
    );
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-lg border bg-card p-4">
          <div className="flex items-center gap-2 text-muted-foreground mb-1">
            ...
            <span className="text-xs font-medium uppercase tracking-wide">Total Planes</span>
          </div>
      ...
      <Show when={!isLoading} fallback={<TableSkeleton columns={columnsMaintenancePlans.length} />}>
        <MainDataTable
